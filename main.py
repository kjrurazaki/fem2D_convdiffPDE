...
def run_2D(model, method, penalty):
      ...
      # Impose BCs
      stiffMat, rhs, u_dir, index_nodes, boundary_nodes = imposeBC(model,
      ...
      #uh = uh + u_dir
      # Residual
      residual = np.linalg.norm(np.zeros((1, model.Nodes)) - np.matmul(model.stiffMat, uh))
...
def grid_run(meshdir, b):
      ...
      list_delta = [None] + list(np.arange(0.01, 0.1, 0.01)) + list(np.arange(0.1, 1.01, 0.1))
      list_diff = list(np.arange(0.01, 0.1, 0.01)) + list(np.arange(0.1, 1.01, 0.1))
      list_penalty = [10 ** i for i in range(0, 10, 1)] + [10 ** i for i in range(10, 16, 2)] + [10 ** i for i in range(30, 40)] 
      ...
      def solve_append(id, model_convection_diff, method, penalty):
            global df_scenarios
            global df_concentration
            uh, residual, residual_bc, info, e = run_2D(model_convection_diff, 
                              method = method,
                              penalty = penalty)
