    ...
    for iel in range(0, model.Nelem):
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                ...
                diffMat[iglob - 1, jglob - 1] = diffMat[iglob - 1, jglob - 1] + \
                    (model.Bloc[iel, iloc] * model.Bloc[iel, jloc] + 
                     model.Cloc[iel, iloc] * model.Cloc[iel, jloc]) * model.Area[iel] * model.Diff[iel]
    return diffMat
...
def transportBuild(model):
    ...
    for iel in range(0, model.Nelem):
        p1 = model.coord[model.triang[iel, 0] - 1,:].reshape(1,-1) # coordinate of first node
        p2 = model.coord[model.triang[iel, 1]  - 1,:].reshape(1,-1) # coordinate of second node
        p3 = model.coord[model.triang[iel, 2]  - 1,:].reshape(1,-1) # coordinate of third node
        for iloc in range(0, 3):
            Num_int = (model.Bloc[iel, iloc] * (p1[:, 0] + p2[:, 0] + p3[:, 0]) +
                       model.Cloc[iel, iloc] * (p1[:, 1] + p2[:, 1] + p3[:, 1]))[0] / 3
            iglob = model.triang[iel, iloc]
            for jloc in range(0, 3):
                jglob = model.triang[iel, jloc]
                transportMat[iglob - 1, jglob - 1] = ( transportMat[iglob - 1, jglob - 1] + 
                    (model.b[iel, 0] * model.Bloc[iel, jloc] + model.b[iel, 1] * model.Cloc[iel, jloc])
                     * (model.Aloc[iel, iloc] + Num_int) * model.Area[iel] )
    return transportMat
        ...
        for iloc in range(0, 3):
            ...
            for jloc in range(0, 3):
                ...
                numdiffMat[iglob - 1, jglob - 1] = ( numdiffMat[iglob - 1, jglob - 1] +
                    (model.Bloc[iel, iloc] * model.Bloc[iel, jloc] * (model.b[iel, 0]**2)
                    + model.Cloc[iel, iloc] * model.Cloc[iel, jloc] * (model.b[iel, 1]**2) +
                    (model.Bloc[iel, iloc] * model.Cloc[iel, jloc] 
                     + model.Bloc[iel, jloc] * model.Cloc[iel, iloc]) * model.b[iel, 0] * model.b[iel, 1]) 
