...
class Model():
    ...
    def build_model(self):
        ...
        self.diffMat, self.transportMat, self.numdiffMat, self.stiffMat = (
                                stiffBuild(self)
        )
    ...
    def convective_flow(self):
        ...
        df['mass_flow'] = (df.apply(lambda a: np.matmul(self.b[0, :], 
                                                        a['Outer Normal']) * 
                                                        a['Concentration'] *
                                                        a['Side Length']**2,
                                                        axis = 1))
    ...
    def sides_dataframe(self):
        ...
        df = pd.DataFrame({'Region': regions, 'Side Length': side_lengths, 
                           'Outer Normal': outer_normals, 'Concentration' : concentration_value}, index=sides)
        return df
    ...
    def plot_boundary(self):
        ...
        handles = [plt.plot([], [], color=color, linewidth=2)[0] for color in colors.values()]
