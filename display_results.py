...
def plot_field_2D(coord, triang, nodal_values, nodal=True, ax=None, label_cbar=None):
    """
    ...
    if nodal == True:
        ...
        suffix = "_triang"
    cbar = plt.colorbar(c, shrink=0.5, format="%.2e", label=label_cbar, ax=ax)
...
def plot_field_3D(
    coord, triang, nodal_values, nodal=True, limit_z=False, figsize=[15, 5]
):
    """
    ...
    plot_subfield(ax2, elev=60, azim=120, limit_z=limit_z)
    plot_subfield(ax3, elev=0, azim=10, limit_z=limit_z)
...
def plot_geometry(coord, triang, show_vertices=True, show_elements=True):
    """
